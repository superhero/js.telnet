...
module.exports = (options = {}) =>
{
  ...
  {
    config.debug_level > 1 && debug(`socket: "close", arguments: "${args}"`);
    queue = [];
    flush();
  });
  ...
  bus.on('ready', () =>
  {
    ...
    config.debug_level > 1 && debug(`executing: "${job.cmd}"`);
  ...
  config.debug && ['connect','drain','error','lookup','end'].forEach((event) =>
    telnet.on(event, (...args) =>
      args.length
      ? config.debug_level > 2 && debug(`socket: "${event}", arguments: "${args}"`)
      : config.debug_level > 2 && debug(`socket: "${event}"`)));
  ...
  config.debug && ['ready','error'].forEach((event) =>
    bus.on(event, (...args) =>
      args.length
      ? config.debug_level > 2 && debug(`module: "${event}", arguments: "${args}"`)
      : config.debug_level > 2 && debug(`module: "${event}"`)));

  // adding debug observers for the module "return" event
  config.debug
  && bus.on('return', (data) => debug(`module: "return":\n${data}`));
